Namespace InterCompanyCreation
    ...
    Public Class clsTable
        ...
        Public Sub FieldCreation()
            AddFields("OITM", "Global", "Global Item", SAPbobsCOM.BoFieldTypes.db_Alpha, 3, , , "N", True)
            AddFields("OADP", "ItemSync", "Sync Item", SAPbobsCOM.BoFieldTypes.db_Alpha, 3, , , "N", True)
            AddFields("OADM", "AddonDis", "Disconnect Addon", SAPbobsCOM.BoFieldTypes.db_Alpha, 3, , , "N", True)
            AddFields("OITM", "Model", "Model", SAPbobsCOM.BoFieldTypes.db_Alpha, 30)
            AddFields("OITM", "Type", "Type", SAPbobsCOM.BoFieldTypes.db_Alpha, 30)
            AddFields("RDR1", "GRNum", "GRNum", SAPbobsCOM.BoFieldTypes.db_Alpha, 15)
            AddFields("RDR1", "GRLine", "GRLine", SAPbobsCOM.BoFieldTypes.db_Alpha, 15)
            AddFields("OITM", "IType", "ItemType", SAPbobsCOM.BoFieldTypes.db_Alpha, 20, , , "-", , {"-,-", "L,Local", "I,Import"})
            AddFields("OITM", "ItemSync", "Item Sync", SAPbobsCOM.BoFieldTypes.db_Alpha, 3, , , "N", , {"Y,Yes", "N,No"})
            AddFields("ORDR", "IComNum", "InterCom Number", SAPbobsCOM.BoFieldTypes.db_Numeric, 10, SAPbobsCOM.BoFldSubTypes.st_None)
            'AddFields("OPOR", "DelNum", "Delivery Number", SAPbobsCOM.BoFieldTypes.db_Alpha, 10)
            AddFields("RDR1", "MarkUp", "MarkUp Percent", SAPbobsCOM.BoFieldTypes.db_Float, 6, SAPbobsCOM.BoFldSubTypes.st_Percentage)
            AddTables("MI_WHSE", "WareHouse List", SAPbobsCOM.BoUTBTableType.bott_NoObject)
            AddFields("@MI_WHSE", "FrmWhse", "From Warehouse", SAPbobsCOM.BoFieldTypes.db_Alpha, 30)
            AddFields("@MI_WHSE", "ToWhse", "To Warehouse", SAPbobsCOM.BoFieldTypes.db_Alpha, 30)
            CompanyList()
            Config()
            Markup()
            AddTables("MI_ITMSL", "ItemGroup Serial Details", SAPbobsCOM.BoUTBTableType.bott_NoObject)
            AddFields("OADP", "ChkMaster", "Enable Master", SAPbobsCOM.BoFieldTypes.db_Alpha, 3, , , "Y", , {"Y,Yes", "N,No"})
            AddFields("OADP", "ChkTran", "Enable Transaction", SAPbobsCOM.BoFieldTypes.db_Alpha, 3, , , "Y", , {"Y,Yes", "N,No"})
            AddFields("OITM", "AddtExp", "Additional Expense", SAPbobsCOM.BoFieldTypes.db_Numeric, 11, SAPbobsCOM.BoFldSubTypes.st_None)
            AddFields("OITM", "StkItem", "Is Stock Item", SAPbobsCOM.BoFieldTypes.db_Alpha, 3, , , "N", , {"Y,YES", "N,No"})
        End Sub
...
#Region "Master Data Creation"
        ...
        Private Sub Markup()
            AddTables("MI_MARKUP", "MARKUP Details", SAPbobsCOM.BoUTBTableType.bott_NoObject)
            AddFields("@MI_MARKUP", "DBName", "DB Name", SAPbobsCOM.BoFieldTypes.db_Alpha, 40)
            AddFields("@MI_MARKUP", "MarkUp", "MarkUp Percent", SAPbobsCOM.BoFieldTypes.db_Float, 6, SAPbobsCOM.BoFldSubTypes.st_Percentage)

        End Sub
        Private Sub Config()
            AddTables("MIPL_SYNC", "DB List Header", SAPbobsCOM.BoUTBTableType.bott_MasterData)
            AddTables("MIPL_SYNC1", "DB List Line", SAPbobsCOM.BoUTBTableType.bott_MasterDataLines)
            AddFields("@MIPL_SYNC1", "Select", "DB Select", SAPbobsCOM.BoFieldTypes.db_Alpha, 3, , , "N", True)
            AddFields("@MIPL_SYNC1", "Sync", "DB Sync", SAPbobsCOM.BoFieldTypes.db_Alpha, 3, , , "N", True)
            AddFields("@MIPL_SYNC1", "DBName", "DB Name", SAPbobsCOM.BoFieldTypes.db_Alpha, 40)

            AddUDO("MISYNC", "InterCompany Sync", SAPbobsCOM.BoUDOObjType.boud_MasterData, "MIPL_SYNC", {"MIPL_SYNC1"}, {"Code", "Name"}, True, False)
        End Sub

        Private Sub CompanyList()
            AddTables("DB_LIST", "Database List", SAPbobsCOM.BoUTBTableType.bott_MasterData)
            AddFields("@DB_LIST", "DBName", "DB Name", SAPbobsCOM.BoFieldTypes.db_Alpha, 30)
            AddFields("@DB_LIST", "UserName", "SAP UserName", SAPbobsCOM.BoFieldTypes.db_Alpha, 25)
            AddFields("@DB_LIST", "Password", "SAP Password", SAPbobsCOM.BoFieldTypes.db_Alpha, 15)
            AddFields("@DB_LIST", "DBUser", "DB User", SAPbobsCOM.BoFieldTypes.db_Alpha, 15)
            AddFields("@DB_LIST", "DBPass", "DB Password", SAPbobsCOM.BoFieldTypes.db_Alpha, 15)
            AddFields("@DB_LIST", "Server", "Server Name", SAPbobsCOM.BoFieldTypes.db_Alpha, 30)
            AddFields("@DB_LIST", "LicServer", "License Server", SAPbobsCOM.BoFieldTypes.db_Alpha, 30)
            AddFields("@DB_LIST", "AddOnName", "Addon Name", SAPbobsCOM.BoFieldTypes.db_Alpha, 30)
            AddFields("@DB_LIST", "Select", "Global ItemSelect", SAPbobsCOM.BoFieldTypes.db_Alpha, 3, , , "Y", , {"Y,Yes", "N,No"})
          
            AddDefaultFormUDO("MILIST", "InterCompany DBLIST", SAPbobsCOM.BoUDOObjType.boud_MasterData, "DB_LIST", {""}, {"Code", "Name"}, True, {"Code", "Name", "U_DBName", "U_UserName", "U_Password", "U_DBUser", "U_DBPass", "U_Server", "U_LicServer", "U_Select", "U_AddOnName"}, True, False)
